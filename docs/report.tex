...
Since the backend of the system was developed by combining 4 separate submodules(Publications, People, Notifications and Reporting) it was ensured that all use cases which
are available to human users are also accessible from external systems, which was a requirement.
...
\section{Mobile Application Framework}
 ...
 \subsubsection{Quality Requirements}
    ...
    \begin{enumerate}
	\item The application could not start.
	...
	\item What little functions are implemented correctly do not receive a wrapped object, or even parameters, as the unit test is hardcoded in.
	\item The android API selected was 6.0 or API level 23. While this is the very latest version possible, it is also the least flexible because only a handful of mobile devices currently support it.
	\item While xml files describing the layout of multiple pages is present, they are never rendered and are imageless and nearly colourless.
	\item The application could not function so the performance of it cannot be measured.
    \end{enumerate}
